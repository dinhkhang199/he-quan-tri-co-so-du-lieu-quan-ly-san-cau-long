...
| **Khung nhìn (Views)** | **4** | `vw_AvailableCourts`, `vw_BookingHistory`, `vw_AdminDashboard`, `vw_AllBookings` |
| **Hàm (Functions)** | **2** | `fn_CalculateBookingCost`, `fn_IsCourtAvailable` |
...
| `Users.PasswordResetCodeHash` | Lưu hash SHA-256 mã OTP khôi phục mật khẩu (`bcms-reset\|<code>`). | `VARBINARY(64) NULL`, không lưu plaintext |
...
| `dbo.sp_ResetPassword` | Kiểm tra OTP, số lần thử (< 5), thời hạn và cập nhật mật khẩu mới. | Đổi mật khẩu thành công sẽ hủy hoàn toàn mã reset (Single-use, chống replay). Phản hồi lỗi phía server được chuẩn hóa generic 400. |

---

## 3. Trạng thái Dịch vụ Email (SMTP Status)

- **DEV / CLASSROOM FALLBACK**: **PASS** (Trong môi trường phát triển / kiểm thử khi chưa cấu hình SMTP máy chủ thật, mã OTP 6 chữ số được trả về trong response phát triển để kiểm thử tự động).
- **SIMULATED SMTP FAILURE**: **PASS** (Khi dịch vụ SMTP gặp sự cố ngoại lệ kết nối, server log lỗi nội bộ và vẫn trả HTTP 200 generic y hệt, triệt tiêu side-channel enumeration).
- **LIVE SMTP**: **NOT RUN** (Hệ thống đã tích hợp sẵn transport `nodemailer` với cấu hình linh hoạt qua biến môi trường `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`; chưa kích hoạt gửi thư thực tế trên máy chủ SMTP ngoài).
...
| **Audit Actor Regression** | `tests/regression/audit_actor.sql` | **PASS** | Xác nhận `ActivityLogs` ghi đúng actor phiên đăng nhập (Manager, Customer). |
| **Product Extension Tests** | `tests/regression/auth_features.sql` | **16/16 PASS** | Kiểm thử đăng ký bắt buộc email, chống trùng lặp, hash OTP, chống tràn số, chống enumeration, khóa 5 lần sai, hết hạn 10p, single-use, đổi mật khẩu và vô hiệu mật khẩu cũ. |
| **Real Concurrent Email (CC-EMAIL-01)** | `tests/concurrency/register_email_session_A.sql` & `tests/concurrency/register_email_session_B.sql` | **PASS** | 2 phiên chạy đồng thời cùng Email: 1 phiên thành công, 1 phiên bị chặn lỗi 50204 qua `sp_getapplock`, `COUNT(*)` = 1. |
| **Real Backup / Restore Hard Assertion** | `tests/regression/backup_restore_runtime.sql` | **PASS** | `BACKUP DATABASE` -> `RESTORE VERIFYONLY` -> `RESTORE` sang `BadmintonCourtManagement_ProductTest_RestoreVerify`. So khớp 100% số dòng và Checksum toàn bộ 5 bảng lõi cùng 5/4/2/17/6 objects. |
...
| **Concurrency CC-01** | `database/11_tests_concurrency_session_A.sql` & `database/12_tests_concurrency_session_B.sql` | **PASS** | Approve tương tranh trên 2 booking overlap -> đúng 1 BOOKED, 1 bị từ chối; không lost update. |
| **Deadlock DL-01/DL-02/DL-03** | `database/13_deadlock_demo_session_A.sql` & `database/14_deadlock_demo_session_B.sql` | **PASS** | Bắt lỗi 1205 deadlock victim và chứng minh lock ordering `Court -> Booking` giải quyết triệt để deadlock. |
| **Lost Update Concurrency** | `tests/concurrency/lost_update_session_A.sql` & `tests/concurrency/lost_update_session_B.sql` | **PASS** | Unsafe mất update (+20k), Fixed với `UPDLOCK` thực hiện tuần tự (+70k). |
| **Dirty Read Concurrency** | `tests/concurrency/dirty_read_session_A.sql` & `tests/concurrency/dirty_read_session_B.sql` | **PASS** | Unsafe đọc dirty 200k, Fixed với `READ COMMITTED` chặn đọc giá trị uncommitted. |
| **Non-repeatable Read** | `tests/concurrency/nonrepeatable_session_A.sql` & `tests/concurrency/nonrepeatable_session_B.sql` | **PASS** | Unsafe đọc 100k rồi 120k, Fixed với `REPEATABLE READ` giữ S-lock ổn định 100k. |
| **Phantom Read Concurrency** | `tests/concurrency/phantom_session_A.sql` & `tests/concurrency/phantom_session_B.sql` | **PASS** | Unsafe tăng số dòng 0 -> 1, Fixed với `SERIALIZABLE` range lock giữ vững 0 -> 0. |
| **TypeScript Typecheck** | `tsc` (Server + Client) | **0 errors** | Kiểm tra tĩnh toàn bộ mã nguồn Server & Client. |
| **ESLint Static Analysis** | `eslint . --max-warnings=0` | **0 warnings / 0 errors** | Tuân thủ toàn bộ quy chuẩn code linting. |
| **App Unit / Integration Tests** | `node --import tsx --test tests/*.test.ts` | **18/18 PASS** | Kiểm thử validator, rate limiter, session db, spError mapping, time parser, và account enumeration side-channel protection (cả forgot password và reset password). |
| **Production Build** | `npm run build` | **PASS** | Build server TSC và Client Vite thành công (dist bundle 264 kB). |
| **Live E2E Smoke Test** | `node dist/server/index.js --serve-client` | **12/12 PASS** | Kiểm thử luồng thực tế trên DB cô lập: Health -> Missing Email Rejected (400) -> Valid Register -> Duplicate (409) -> Login -> Me -> Search Court -> Logout -> Forgot Password -> OTP Reset -> Re-login thành công. |
